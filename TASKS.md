...
- [ ] IMP-AUTO-20260314-01 | Add explicit argparse examples/help text to lightweight hygiene helpers (`task_hygiene_check.py`, `markdown_link_check.py`) to reduce avoidable command-shape errors in autonomous runs (impact: lowers operator/agent misuse and failed sweep starts).
- [ ] IMP-AUTO-20260314-02 | Backfill canonical frontmatter for the remaining legacy `knowledge/reports/*.md` artifacts that still index as `source: unknown` / missing `title,date,source,ingest_from`, and add regression coverage so daily information-model sweeps do not keep surfacing avoidable metadata drift (impact: restores deterministic report metadata quality across the full library).
- [ ] IMP-AUTO-20260315-01 | Restore or retire the missing authoritative `INFORMATION_MANAGEMENT_PROCESS_V1.md` referenced in `PROCESS_REGISTRY.md`, and align the daily information-model sweep with an actual source-of-truth runbook (impact: removes process-discovery dead link and prevents autonomous maintenance from depending on an absent process doc).
...
- [ ] SEC-AUTO-20260314-01 | Reconfirm and document current owner disposition for recurring `security.trust_model.multi_user_heuristic` after 2026-03-14 nightly audit, including whether Telegram group access remains within a single trusted-operator boundary or now warrants split gateways / tighter sandbox scope.
- [ ] SEC-AUTO-20260314-02 | Make nightly macOS host-network evidence deterministic without elevation-path ambiguity: codify approved absolute-path collectors for listener checks and an explicit fallback/escalation note for PF status when `/sbin/pfctl -s info` is permission-gated in cron.

## Triage
- [ ] OPS-2026-066 | [Jobs Review 2026-03-16] Validate JOB-CI-001 cron delivery model against OpenClaw 2026.3.11 breaking change (isolated cron agent-send no longer permitted): confirm existing CI-001 cron jobs do not rely on ad-hoc sends, and update JOB-CI-001 execution profile in `JOB_MARKET_MODEL_V1.md` to reflect the new delivery constraint (impact: prevents silent CI delivery failures post-update).
- [ ] OPS-2026-067 | [Jobs Review 2026-03-16] Tighten JOB-SEC-001 job record with a stale-finding SLA (e.g. recurring warnings unresolved >7 days trigger mandatory owner-approved disposition) and add explicit disposition trigger to `JOB_MARKET_MODEL_V1.md`; apply first to `SEC-AUTO-20260307-01` (multi_user_heuristic) and `SEC-AUTO-20260309-02` (pfctl posture gap) which have been open since early March (impact: converts detection without closure into accountable dispositions).
- [ ] OPS-2026-068 | [Jobs Review 2026-03-16] Formally retire proof-case job bundles JOB-TM-001, JOB-TM-002, JOB-DEL-001, JOB-GOV-001: mark STATE.md as closed, move `jobs/<ID>/` to `archive/jobs/` or equivalent, and record standardization outcome (intra-Lyra handoff protocol validated for same-runtime multi-lane use) in a governance artifact (impact: clears completed work from active job namespace and preserves institutional learning).
- [ ] OPS-2026-069 | [Jobs Review 2026-03-16] Validate and close or formally re-accept SEC-AUTO-20260307-01 (recurring multi_user_heuristic disposition) and SEC-AUTO-20260309-02 (non-elevated pfctl posture evidence gap) — both 9+ days open; requires owner-approved disposition with explicit expiry/reopen triggers per JOB-SEC-001 (impact: clears the longest-running open security finding backlog items).
...
- [ ] IMP-ERR-20260315-ARCHIVED-REPO-MISUSE-01 | Convert ERR-2026-03-15-ARCHIVED-REPO-MISUSE into linked TDE improvement execution artifact (Improvement P3 test case)
  <!-- tde:metadata {"objective_id": "OBJ-IMPROVEMENT-EXECUTION", "stage_id": "implementation", "activated_at": "2026-03-16T00:36:45Z", "activated_by": "control-tower-overnight-synthesis-2026-03-16", "linked_error": "ARCHIVED_REPO_MISUSE_ERROR_REPORT_2026-03-15.md"} -->
...
## Done
- [x] TASK-20260314-VEGA-PXS-BOUNDARY-PASS | Make Vega/PXS boundary PASS so downstream `pxs` consumption can proceed safely — closed 2026-03-16: all criteria met for Phase 1; E2 scoped to filesystem-tool enforcement (exec intentionally open per Peter decision); acceptance sheet signed off; boundary v1 active
  <!-- tde:metadata {"objective_id": "OBJ-TDE-FOUNDATION", "stage_id": "acceptance-gate", "activated_at": "2026-03-15T00:37:54Z", "activated_by": "control-tower-overnight-synthesis-2026-03-15", "closed_at": "2026-03-16T20:26:00Z", "closed_by": "peter-decision", "decision": "E2 criterion scoped to Phase 1 — process discipline is Phase 1 control; exec sandboxing deferred to long-term security phase", "evidence": "governance/VEGA_ACCEPTANCE_TEST_RUN_SHEET_V1.md"} -->
- [x] TASK-20260315-CP-TDE-ASSIGNMENT-ACCEPTANCE | Close Control Panel → TDE assignment acceptance / silent-limbo gap (thin-slice: acceptance states, persistence, explicit result contracts) — closed 2026-03-16 overnight: all 5 acceptance states proven, 21-case pytest suite + 7-case integration test suite passing, end-to-end probe confirmed, no silent limbo possible
  <!-- tde:metadata {"objective_id": "OBJ-TDE-FOUNDATION", "stage_id": "implementation", "activated_at": "2026-03-15T01:35:00Z", "activated_by": "control-tower-overnight-synthesis-2026-03-15", "closed_at": "2026-03-16T04:00:00Z", "evidence": "products/task-management/04-execution/TDE_ASSIGNMENT_ACCEPTANCE_TEST_EVIDENCE_2026-03-16.md"} -->
