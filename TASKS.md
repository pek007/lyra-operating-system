...
## Inbox
- [ ] OPS-2026-079 | Publish and apply canonical repo authority for active code-bearing products: record canonical local clone paths, make repo-root confirmation explicit before sync/rebase/push actions, and verify that TDE / Lyra OS work uses `repos/lyra-operating-system` while Control Panel work uses `repos/control-panel` (impact: prevents wrong-clone Git decisions and reduces sync/rebase risk).
- [ ] OPS-2026-080 | Define and adopt a lightweight Git sync cadence rule for active code-bearing products so high-signal work reaches GitHub more frequently and large divergence stacks stop building unnoticed (impact: lowers reconciliation risk and improves recovery posture).
...
- [ ] OPS-2026-078 | Establish a stable cross-runtime communication loop for Lyra OS <-> PX: adopt inbox/outbox handoff structure, define pickup cadence and handled-state rules, and decide whether/when to add cron-backed inbox checks on each side.
  ...
  - [ ] Next | Decide whether to implement the Lyra OS-side inbox review cron now or wait until the PX verify trial completes.
...
- [ ] OPS-2026-073 | Design a lightweight intra-Lyra cross-session handoff protocol that replaces copy-paste as the default coordination method while remaining lighter than the Vega cross-domain handoff model.
  ...
  - [x] 2026-03-10 | Created real proof-case job bundle `jobs/JOB-TM-001/` and executed the first live Task Management handoff via `sessions_send`; receiving lane returned `result` and updated `jobs/JOB-TM-001/STATE.md`. Evidence: `knowledge/evidence/2026-03-10__ops-2026-073-task-management-proof-case-live-run.md`.
  - [x] 2026-03-10 | Ran second bounded live Task Management proof case (`jobs/JOB-TM-002/`) and confirmed repeatability in the same lane; recommendation from receiving lane: provisionally standardize for same-runtime intra-Lyra handoffs only, and require 1-2 more live runs in different lanes before broader standardization. Evidence: `knowledge/evidence/2026-03-10__ops-2026-073-task-management-repeatability-run.md`.
  - [x] 2026-03-10 | Ran Governance transfer proof case with `jobs/JOB-GOV-001/`; result: initial cross-lane transfer evidence established, `A-002-I1` upgraded from placeholder to a real bounded Governance initiative, and protocol status advanced to default for same-runtime intra-Lyra handoffs. Evidence: `knowledge/evidence/2026-03-10__ops-2026-073-governance-transfer-proof-case.md`.
  - [x] 2026-03-10 | Ran Delivery transfer proof case with `jobs/JOB-DEL-001/`; result: broader same-runtime multi-lane standardization threshold reached, A-006 scorecard gained first concrete baseline note, and protocol status advanced to standardized for same-runtime intra-Lyra handoffs. Evidence: `knowledge/evidence/2026-03-10__ops-2026-073-delivery-transfer-proof-case.md`.
  - [ ] Next | Optionally run one harsher confirmation case (Security) if we want to test whether the protocol needs any lane-specific tightening under stricter operating conditions.
- [ ] OPS-2026-070 | Operationalize `MEMORY_PROCESS_V1.md`: validate live memory retrieval behavior, define the first activation-class map for priority memory artifacts, and publish a phased implementation roadmap for agent/session/job/knowledge/coordination memory under Control Panel ownership.
  ...
  - [x] 2026-03-10 | Validated live retrieval/index behavior via `openclaw status --all`, `openclaw plugins list`, `openclaw memory status --deep`, and `openclaw memory index --verbose`; result: `memory-core` was loaded but both agents remained at `0 indexed files / 0 chunks` because file sync was skipped in FTS-only mode with no embedding provider. Evidence: `knowledge/evidence/2026-03-10__ops-2026-070-memory-retrieval-baseline-validation.md`.
  - [x] 2026-03-10 | Applied memory-search provider fix (`agents.defaults.memorySearch.provider=local`), re-indexed, and validated successful retrieval from `MEMORY.md` and daily memory files. Evidence: `knowledge/evidence/2026-03-10__ops-2026-070-memory-retrieval-fix-and-validation.md`.
  - [x] 2026-03-10 | Added `jobs` to `agents.defaults.memorySearch.extraPaths`, re-indexed, and validated that job-memory template artifacts under `jobs/JOB-TEMPLATE/` are now retrievable via `memory_search`. Evidence: `knowledge/evidence/2026-03-10__ops-2026-070-job-memory-extra-path-validation.md`.
  - [ ] Next | Populate at least one real active job bundle and validate recall on live job-state content before expanding retrieval scope further to `knowledge/distilled/`.
