...
- [ ] IMP-AUTO-20260305-02 | Add a lightweight “referenced script exists” guard for cron/runbook command strings to catch broken tooling pointers before scheduled sweeps fail (impact: fail-fast on operational doc drift).
  - [ ] 2026-03-06 | Opened as next reliability bottleneck after schema-gate hardening; implementation queued next (scan runbook/cron command strings for local script path existence and fail CI/validator on missing targets).
